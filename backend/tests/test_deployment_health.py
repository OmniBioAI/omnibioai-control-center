...
Unit tests for control_center.deployment_health's docker-compose parsing:
service inventory extraction, repository-ownership inference (build
context, image, dockerfile path segment, static mapping fallback, and the
never-guess/never-invent-a-human-owner guarantees), service category
resolution, the soft/hard depends_on dependency model (including malformed
input handling), image-reference/tag parsing, port extraction, the
secret/PHI-safe to_public_dict() allowlisted serialization, typed error
handling for missing/malformed compose files, and completeness reporting.
...
def _service(inventory, service_id):
    """The inventory's service entry with the given service_id."""
    return next(s for s in inventory.services if s.service_id == service_id)


def _dep(inventory, from_service, to_service):
    """The inventory's dependency edge from `from_service` to `to_service`."""
    return next(
...
def test_valid_compose_parses_all_services():
    """Every service defined in a valid compose file is parsed into the inventory, with no warnings."""
...
def test_service_inventory_fields():
    """A parsed service's display_name, build_configured, healthcheck_configured, ports, and dependency_count reflect its compose definition."""
...
def test_healthcheck_and_build_flags():
    """A service with an image and healthcheck (mysql) is flagged healthcheck_configured=True, build_configured=False."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_baseline_source_recorded():
    """The inventory's baseline_source reflects the caller-supplied value, defaulting to UNKNOWN when omitted."""
...
def test_ownership_from_build_context_wins_evidence_tier():
    """A service with a build context under an omnibioai-* directory is attributed to that repo via COMPOSE_BUILD_CONTEXT evidence."""
    inventory = parse_compose_text(_VALID_COMPOSE)
    auth = _service(inventory, "auth-service")
...
def test_ownership_from_image_when_no_build_context():
    """With no build context, a service's repository is inferred from its image name via COMPOSE_IMAGE evidence."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_ownership_from_static_mapping_fallback():
    """An image whose name doesn't match an omnibioai-* pattern still resolves via the STATIC_OWNERSHIP_MAPPING fallback."""
    compose = """
services:
  toolserver:
...
def test_ownership_via_dockerfile_path_segment():
    """A build.dockerfile path containing an omnibioai-* segment supplies ownership even when the build context itself doesn't."""
    compose = """
services:
  rag:
    build:
...
def test_ownership_relative_build_context_uses_deployment_repository():
    """A relative build context (".") is attributed to the caller-supplied deployment_repository."""
    inventory = parse_compose_text(_VALID_COMPOSE, deployment_repository="omnibioai-studio")
...
def test_ownership_relative_build_context_without_deployment_repository_is_unknown():
    """A relative build context with no deployment_repository supplied leaves ownership unknown rather than guessing."""
...
def test_unknown_ownership_stays_unknown_not_guessed():
    """A service whose image matches no known pattern has repository=None rather than a fabricated guess."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_ownership_never_invents_human_owner():
    """Every resolved repository name is either None or an omnibioai-* repo -- never a fabricated human owner."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_known_category_resolved():
    """A recognized service_id (mysql, auth-service, api-gateway) resolves to its correct ServiceCategory."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_unrecognized_service_id_category_is_unknown_not_guessed():
    """An unrecognized service_id resolves to ServiceCategory.UNKNOWN rather than a guessed category."""
    inventory = parse_compose_text(_VALID_COMPOSE)
    mystery = _service(inventory, "mystery-service")
...
def test_depends_on_list_syntax_is_soft_by_default():
    """A plain-list depends_on entry (no condition) is classified SOFT."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_depends_on_mapping_syntax_service_healthy_is_hard():
    """A mapping-syntax depends_on with condition: service_healthy is classified HARD."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_depends_on_mapping_syntax_service_started_is_soft():
    """A mapping-syntax depends_on with condition: service_started is classified SOFT."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_depends_on_completed_successfully_is_hard():
    """A depends_on with condition: service_completed_successfully is classified HARD."""
    compose = """
services:
  deploy-verify:
...
def test_dependency_evidence_present_on_every_edge():
    """Every dependency edge carries COMPOSE_DEPENDS_ON evidence whose detail names the target service."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_depends_on_does_not_infer_from_coexistence():
    """Two services with no explicit depends_on between them get no inferred edge, even if they coexist in the same compose file."""
...
def test_unknown_dependency_target_recorded_with_warning_not_failure():
    """A depends_on referencing a service_id not defined elsewhere in the file still produces an edge, flagged with an unknown_dependency_target warning."""
    compose = """
services:
  a:
    image: something
...
def test_malformed_depends_on_marks_dependencies_unknown():
    """A depends_on value that's neither a list nor a mapping marks the service's dependencies as unknown, not silently empty."""
    compose = """
services:
  a:
    image: something
...
def test_malformed_depends_on_mapping_value_recorded_with_warning():
    """A depends_on mapping whose value isn't itself a mapping still produces a SOFT edge, flagged with a malformed_depends_on warning."""
    compose = """
services:
  a:
    image: something
...
def test_malformed_depends_on_list_entry_skipped_with_warning():
    """A non-string depends_on list entry is skipped (no edge produced) and flagged with a malformed_depends_on warning."""
    compose = """
services:
  a:
    image: something
...
def test_image_reference_parsing(raw, expected_registry, expected_repository, expected_tag, expected_digest):
    """parse_image_reference() splits a raw image string into registry/repository/tag/digest, and preserves the original raw string."""
...
def test_digest_image_reference():
    """An image pinned by @sha256 digest has that digest recorded, no tag, and is_untagged=False (pinned, not unknown)."""
...
def test_untagged_image_reference():
    """An image with no tag and no digest is is_untagged=True and not treated as a "latest" tag."""
...
def test_latest_tag_not_treated_as_verified():
    """An explicit ":latest" tag is recorded as such and flagged is_latest_tag=True, distinct from being untagged."""
...
def test_variable_containing_image_reference_does_not_crash():
    """An image reference containing an unresolved ${VAR} substitution (in the tag or the whole reference) parses without raising, flagged has_variable=True."""
...
def test_image_metadata_on_service_reflects_tag_parsing():
    """A service's .image metadata reflects the parsed repository and tag from its compose image string."""
    inventory = parse_compose_text(_VALID_COMPOSE)
    mysql = _service(inventory, "mysql")
...
def test_ports_extraction_handles_int_dict_and_string_forms():
    """Port entries in int, "host:container", and {target, published} dict form are all extracted; non-port entries (strings, null, bool) are skipped."""
    compose = """
services:
  a:
    image: something
...
def test_ports_extraction_ignores_non_list_value():
    """A non-list "ports" value is ignored, leaving ports empty rather than raising."""
    compose = """
services:
  a:
    image: something
...
def test_build_shorthand_string_form_used_for_ownership_and_build_configured():
    """A build: "<path>" shorthand (string, not a mapping) still sets build_configured=True and supplies ownership evidence."""
    compose = """
services:
  a:
...
def _flatten_strings(obj) -> list[str]:
    """Every string value found anywhere in a nested dict/list structure, recursively."""
    out: list[str] = []
...
def test_serialized_output_excludes_environment_and_secret_values():
    """to_public_dict()'s JSON serialization never contains any environment variable name or secret value from the compose file."""
    inventory = parse_compose_text(_VALID_COMPOSE, deployment_repository="omnibioai-studio")
...
def test_serialized_output_excludes_absolute_developer_paths_with_no_repo_segment():
    """An absolute build context with no omnibioai-* segment leaves repository unknown, and its raw path never appears in the serialized output."""
    compose = """
services:
  weird:
    build:
...
def test_absolute_path_containing_repo_segment_is_normalized_not_leaked():
    """An absolute build context containing an omnibioai-* segment resolves ownership from that segment, but the raw absolute path itself never appears in serialized output."""
...
def test_serialized_output_excludes_container_ids_and_backend_handles():
    """to_public_dict()'s JSON serialization never mentions container/backend-handle/scheduler-related identifiers."""
    inventory = parse_compose_text(_VALID_COMPOSE, deployment_repository="omnibioai-studio")
    payload = json.dumps(inventory.to_public_dict())
...
def test_to_public_dict_is_allowlisted_not_recursive_environment_dump():
    """A service's to_public_dict() exposes exactly the allowlisted field set, not a recursive dump of its raw compose definition (which could leak environment values)."""
    inventory = parse_compose_text(_VALID_COMPOSE)
...
def test_missing_compose_file_raises_typed_error(tmp_path):
    """load_compose_file() on a nonexistent path raises DeploymentHealthUnavailable with code "compose_not_found"."""
...
def test_malformed_yaml_raises_typed_error():
    """Unparseable YAML raises DeploymentHealthUnavailable with code "invalid_yaml"."""
    with pytest.raises(DeploymentHealthUnavailable) as excinfo:
...
def test_root_not_mapping_raises_typed_error():
    """A YAML document whose root is a list, not a mapping, raises DeploymentHealthUnavailable with code "root_not_mapping"."""
    with pytest.raises(DeploymentHealthUnavailable) as excinfo:
...
def test_services_key_not_a_mapping_raises_typed_error():
    """A "services" key whose value isn't a mapping raises DeploymentHealthUnavailable with code "invalid_services_key"."""
    with pytest.raises(DeploymentHealthUnavailable) as excinfo:
...
def test_missing_services_key_returns_empty_inventory_not_error():
    """A compose file with no "services" key at all returns an empty inventory with a missing_services_key warning, not an error."""
...
def test_empty_services_returns_empty_inventory_with_warning():
    """An empty "services: {}" mapping returns an empty inventory with an empty_services warning."""
...
def test_empty_document_returns_empty_inventory():
    """An entirely empty compose document returns an empty inventory with an empty_document warning."""
...
def test_invalid_service_definition_skipped_with_warning():
    """A service definition that's null (not a mapping) is skipped, leaving the other services intact, flagged with an invalid_service_definition warning."""
    compose = """
services:
  good:
    image: mysql:8.0
...
def test_load_compose_file_reads_explicit_path(tmp_path):
    """load_compose_file() reads and parses a compose file from an explicit filesystem path."""
...
def test_serialization_is_deterministic():
    """Parsing the same compose text twice produces byte-identical serialized output."""
...
def test_metadata_completeness_known_service():
    """A service with fully-resolved metadata (auth-service) reports is_complete=True with no missing fields."""
    inventory = parse_compose_text(_VALID_COMPOSE)
    auth = _service(inventory, "auth-service")
...
def test_metadata_completeness_partial_service():
    """A service with unresolved repository and category (mystery-service) reports is_complete=False and names both fields as missing."""
    inventory = parse_compose_text(_VALID_COMPOSE)
    mystery = _service(inventory, "mystery-service")
